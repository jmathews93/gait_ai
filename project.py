...
def dnn_apply(X_train_values, y_train_values, X_test_values, y_test_values):
    ...
    model.add(Dense(int(inputs[1]*.75), activation='sigmoid'))
    model.add(Dropout(0.1))
...
random_forest(X_trainScaled, np.argmax(onehot_encoded, axis=-1), X_testScaled, np.argmax(onehot_encoded_test, axis=-1), 100)
svm_apply(X_trainScaled, np.argmax(onehot_encoded, axis=-1), X_testScaled, np.argmax(onehot_encoded_test, axis=-1))
mlp_apply(X_trainScaled, np.argmax(onehot_encoded, axis=-1), X_testScaled, np.argmax(onehot_encoded_test, axis=-1))

dnn_apply(X_trainScaled, onehot_encoded, X_testScaled, onehot_encoded_test)
